...
my $fhosts_pve = "/etc/pve/priv/hosts";
my $fhosts = "/etc/hosts";
...
sub rsync_hosts {
    ...
    my @rsync = ('/usr/bin/rsync', $fhosts_pve, $fhosts);
    
    my $raw = PVE::Tools::file_get_contents('/etc/pve/corosync.conf');
    my $conf = PVE::Corosync::parse_conf('corosync.conf', $raw);
    my $nodes = $conf->{main}->{nodelist}->{node};
    foreach my $node (keys %$nodes) {
        if ($node ne $local_node) {
            my @remcmd = ('/usr/bin/ssh', $node, '--');
            PVE::Tools::run_command([@remcmd, @rsync]); 
        }
    }
}

sub rsync_local_hosts {
    my @rsync = ('/usr/bin/rsync', $fhosts_pve, $fhosts);
    PVE::Tools::run_command([@rsync]);
}
...
sub ssh_copy_id {
    ...
    $exp->spawn('ssh-copy-id', '-i', '/root/.ssh/id_rsa', "root\@$host");
    $exp->log_stdout(0);    # 关闭目标程序的输出显示
...
__PACKAGE__->register_method ({
	...
	my $filename = $param->{filename};
        ...
        write_debug("/root/debug", $>);
        print "$>\n";
	# test EUID
	$> == 0 || die "Error2: Authorization key must be generated as root user.\n";
...
__PACKAGE__->register_method ({
	...
	my $local_ip_address = PVE::Cluster::remote_node_ip($nodename);
        ...
        write_hosts($fhosts_pve, "127.0.0.1 localhost.localdomain localhost\n$local_ip_address $nodename");
        rsync_local_hosts();
...
__PACKAGE__->register_method ({
	...
	my ($param) = @_;
        ...
        write_hosts($fhosts, "$address $host\n");
	...
	my $cmd = ['ssh', $host, '-o', 'BatchMode=yes',
